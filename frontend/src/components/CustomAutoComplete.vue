              ...
              >
                <template v-if="!optionGroupLabel">
                  <li
                    ...
                    :key="getOptionRenderKey(item)"
                    @click="selectItem($event, item)"
                    role="option"
                    v-ripple
                    :data-index="getOptionIndex(i, getItemOptions)"
                  >
                    <slot
                      name="item"
                      :item="item"
                      :index="getOptionIndex(i, getItemOptions)"
                      >{{ getItemContent(item) }}</slot
                    >
                  </li>
                </template>
                <template v-else>
                  ...
                  >
                    ...
                    <li
                      v-for="(item, j) of getOptionGroupChildren(optionGroup)"
                      class="p-autocomplete-item"
                      :key="j"
                      @click="selectItem($event, item)"
                      role="option"
                      v-ripple
                      :data-group="i"
                      :data-index="getOptionIndex(j, getItemOptions)"
                    >
                      <slot
                        name="item"
                        :item="item"
                        :index="getOptionIndex(j, getItemOptions)"
                        >{{ getItemContent(item) }}</slot
                      >
                    </li>
                  </template>
...
export default {
  ...
  watch: {
    suggestions() {
      if (this.searching) {
        if (this.suggestions && this.suggestions.length) this.showOverlay();
  ...
  methods: {
        ...
        switch (event.which) {
          ...
          case 40:
            if (highlightItem) {
              ...
              if (
                DomHandler.hasClass(highlightItem, "p-autocomplete-item-group")
              ) {
    ...
    selectHighlightItem(event, item) {
      if (this.optionGroupLabel) {
        ...
        this.selectItem(
          event,
          this.getOptionGroupChildren(optionGroup)[item.dataset.index]
        );
      } else {
    ...
    findNextItem(item) {
      ...
      if (nextItem)
        return DomHandler.hasClass(nextItem, "p-autocomplete-item-group")
    ...
    findPrevItem(item) {
      ...
      if (prevItem)
        return DomHandler.hasClass(prevItem, "p-autocomplete-item-group")
